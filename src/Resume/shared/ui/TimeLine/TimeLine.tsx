...
import { Lang, langContext, T } from '../ToggleLanguage/ToggleLanguage';
import { SvgProps } from '@react-three/drei';

type Component = typeof HStack;
type ElRef = ElementRef<Component> | null;

export interface TimeProps extends ComponentProps<Component> {
    time: Date
}
export const Time = (props: TimeProps) => {
    ...
    return (
        <HStack className={classNames(cls.time, [className])}
            datetime={`${time.getFullYear()}-${String(time.getMonth() + 1).padStart(2, "0")}`}
            tag="time" {...otherProps}
        >
            {
            `${time.toLocaleString(lang === "ru" ? 'ru-RU' : "en-EN", { month: 'short' })}`
                .charAt(0).toUpperCase() +
                `${time.toLocaleString(lang === "ru" ? 'ru-RU' : "en-EN", { month: 'short' })}`.slice(1)
                +  " " + time.getFullYear()
            }
        </HStack>
    )
...
export interface TimeLineProps extends ComponentProps<Component> {
    time: Ed["time"]
    timeProps?: TimeProps
