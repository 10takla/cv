...
const Hightlight = (props: HightlightProps, ref: ForwardedRef<ElRef>) => {
    ...
    const isHightligt = useMemo(() => decodeURIComponent(location.hash) === `#${id}`, [location.hash, id]);
    ...
    useEffect(() => {
        const handler = () => navigate({ pathname: location.pathname }, { replace: true });
        isHightligt && window.addEventListener("mousedown", handler);
